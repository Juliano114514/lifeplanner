...
class PlanFlowReducerTest {
  ...
  @Test
  fun goOut_followUp_visibleWhenTimeSlotSelected_hiddenForStayHome() {
    ...
    assertEquals(listOf("出去玩", "出去办事"), state.currentAnswer()?.subSelections)
  ...
  @Test
  fun work_followUp_skippedForRest_clearsSubSelectionOnMainChange() {
    ...
    assertEquals(listOf("在家"), state.currentAnswer()?.subSelections)
  ...
  @Test
  fun breakfast_followUp_onlyWhenCookAtHome_supportsMultiSelect() {
    ...
    assertEquals(listOf("有菜了", "要去买菜"), state.currentAnswer()?.subSelections)
